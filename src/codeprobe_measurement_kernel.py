...
# CE04 revises evidence coverage, not any numerical counting rule.
TRAVERSAL_OBSERVATIONS = frozenset({
    "python.callable.decision_point_count",
    "python.callable.mccabe_complexity",
    "python.callable.max_control_nesting_depth",
})


def specification_version(observation_id: str) -> str:
    """Version evidence semantics separately from the complete kernel build."""
    return "1.1.1" if observation_id in TRAVERSAL_OBSERVATIONS else "1.1.0"
...
def observation(
    ...
    *,
    ...
    specification_id = f"codeprobe-observation/{observation_id}/{specification_version(observation_id)}"
    record = {
...
def validate_observation_record(record: Mapping[str, Any]) -> None:
    ...
    expected_specification = f"codeprobe-observation/{observation_id}/{specification_version(observation_id)}"
...
def _callable_traversal_evidence(intake: IntakeRecord, node: ast.AST,
                                 start: int, end: int) -> Dict[str, Any]:
    """Conservatively cover root definition expressions without changing size.

    The first decorator *AST expression* can start after its @/opening grouping
    line. Coordinates cover the admitted traversal, not a minimal slice, token
    highlight or a promise that the excerpt can be compiled independently.
    """
    first = start
    for decorator in getattr(node, "decorator_list", ()):
        lo = getattr(decorator, "lineno", None)
        hi = getattr(decorator, "end_lineno", None)
        if (type(lo) is not int or type(hi) is not int
                or not 1 <= lo <= hi <= end <= len(intake.physical_lines)):
            raise MeasurementError("CE04-LOC-001", "Decorator evidence coordinates are unavailable or invalid.")
        first = min(first, lo)
    return _span_evidence(intake, first, end, scope="python_callable_traversal")
    ...
    for ordinal, node in enumerate(functions, 1):
        ...
        if start < 1 or end < start:
            ...
            records.append(observation(
                ...
                evidence_ids=evidence_ids, parser="CPython_ast_callable_visitor/1.1.1",
        ...
        traversal_span = _callable_traversal_evidence(intake, node, start, end)
        evidence.append(traversal_span)
        ...
        common = {
            ...
            "parser": "CPython_ast_callable_visitor/1.1.1",
            "limitations": (
                "Nested function, lambda and class nodes, including their entire subtrees, are excluded from the enclosing callable.",
                "Root definition expressions (including defaults, annotations and decorators) are traversed syntactically, not as runtime control flow.",
                "The control-node convention is finite and is not a universal CFG metric.",
            ),
        }
        traversal_common = dict(common, evidence_ids=[traversal_span["evidence_id"]])
        span_common = dict(common, limitations=(
            "Physical span runs from the definition line through end_lineno; root decorators are excluded.",
            "Blank lines, comments and nested definitions within that span are included; this is not executable SLOC.",
        ))
        records.extend([
            observation("python.callable.physical_span_lines", value=end - start + 1,
                        measurement_scale="ratio", unit="line", **span_common),
            observation("python.callable.decision_point_count", value=metrics.decision_points,
                        measurement_scale="ratio", unit="decision_point", **traversal_common),
            observation("python.callable.mccabe_complexity", value=1 + metrics.decision_points,
                        measurement_scale="ratio", unit="complexity_unit", **traversal_common),
            observation("python.callable.max_control_nesting_depth", value=metrics.max_depth,
                        measurement_scale="ratio", unit="level", **traversal_common),
        ])
    return records
...
def validate_kernel_result(result: Mapping[str, Any]) -> None:
    ...
    for item in source_evidence:
        ...
        if item["scope"] not in {"whole_artifact", "python_callable", "python_callable_traversal"}:
    ...
    for record in observations:
        ...
        if record["observation_id"] in TRAVERSAL_OBSERVATIONS:
            expected_scope = "python_callable_traversal"
...
def verify_result_against_catalogue(result: Mapping[str, Any], catalogue: Mapping[str, Any]) -> None:
    ...
    for observation_id, records in records_by_id.items():
        ...
        if any(record["specification_id"] != specification["specification_id"]
               or record["instrumentation"]["algorithm_version"] != specification["algorithm"]["version"]
               for record in records):
            raise ValueError(f"catalogue specification or algorithm identity mismatch: {observation_id}")
