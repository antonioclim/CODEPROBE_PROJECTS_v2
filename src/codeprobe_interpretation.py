...
_INVENTORY = strict_json(_read('research/ce04/component-inventory.v1.json'))
...
if any(_LOADED.get(p) != h for p, h in _INVENTORY['inherited_sha256'].items()):
    raise InterpretationError('declared unchanged baseline component changed')
if any(_LOADED.get(p) != entry['candidate_sha256']
       for p, entry in _INVENTORY.get('revised_components', {}).items()):
    raise InterpretationError('declared revised component changed')
...
def build_identity() -> dict[str, Any]:
    """Identify admitted bytes without launching Git or trusting parent metadata.

    Repository commit and tree anchors belong in an external release-provenance
    receipt created after the exact integration commit exists. Keeping them
    outside this component avoids process-launch side effects and avoids a
    self-referential commit claim inside the commit being identified.
    """
    _unchanged()
    return {
        'schema': 'codeprobe-build-identity/v1',
        'component_sha256': dict(_LOADED),
        'implementation_digest': digest(canonical(_LOADED)),
        'git_anchor_commit': None,
        'git_anchor_tree': None,
        'checkout_state': 'unversioned_source',
        'runtime': {
        ...
        'qualification': (
            'File-set consistency only. Commit and tree anchors require an '
            'external release-provenance receipt; neither source authorship '
            'nor a hostile runtime is authenticated.'
        ),
    }
...
def _uncertainty(record: dict[str, Any] | None, sensitivity: str) -> dict[str, Any]:
    return {'measurement_interval': None, 'measurement_interval_status': 'not_estimated',
            'computational_scope': 'finite_ce04_measurement_contract', 'runtime_dependence': 'declared_parser_and_runtime',
