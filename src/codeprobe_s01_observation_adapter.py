...
def adapt_observation(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    Source evidence uses S01-safe hashed transport identifiers, with the exact
    kernel locator preserved in source_id. The kernel owns the span catalogue. S04 may enrich this boundary, but it may
    ...
    output.update({
        ...
        "source_evidence": [
            {
                # S01 stable identifiers have a narrower grammar than kernel
                # locators. Keep the original verbatim in source_id.
                "evidence_id": "kernel_evidence." + hashlib.sha256(evidence_id.encode("utf-8")).hexdigest(),
